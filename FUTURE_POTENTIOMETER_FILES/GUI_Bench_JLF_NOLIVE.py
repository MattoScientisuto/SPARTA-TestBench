...
def read_load_cell():
    ...
    with nidaqmx.Task() as ai_task:
        ...
        ai_task.timing.cfg_samp_clk_timing(rate=100,sample_mode=AcquisitionType.CONTINUOUS)
        ...
        with open(f'.\\data_output\\cpt\\{todays_date}\\{csv_list[0]}', 'a', newline='') as file:
            ...
            for i in range(cpt_samples):
                ...
                print(continued_timestamp)

                strain_data.append(newton)
                r_count+=1
                
                nolive_timestamp.append(continued_timestamp)
                nolive_pounds.append(strain)
                nolive_raw.append(strain)

                # Write current value to CSV
                # Real-time so that the GUI plot can keep up
                # writer.writerow([continued_timestamp, cdepth, strain, strain])
        ...
        with open(f'.\\data_output\\cpt\\{todays_date}\\{csv_list[0]}', 'a', newline='') as file:
            ...
            # Determine the length of the other lists
            list_length = min(len(nolive_timestamp), len(nolive_pounds), len(nolive_pounds))
            
            # Iterate over the lists and write them row by row
            for i in range(list_length):
                # Create the row data
                row_data = [nolive_timestamp[i], nolive_pounds[i], nolive_pounds[i], nolive_pounds[i]]
                
                # Append live_ards if available, otherwise append an empty string
                if i < len(live_ards):
                    row_data.insert(1, live_ards[i])
                else:
                    row_data.insert(1, '')  # Insert empty string if live_ards data is not available
...
def read_arduino_data():
    ...
    while lc_running:
        ard_data = read_from_arduino()
        live_ards.append(ard_data)
        print(ard_data)
...
def load_cell_run():
    ...
    thread_ard = Thread(target=read_arduino_data)
    thread_cpt.start()
    thread_ard.start()
...
def read_from_arduino():
    ...
    """
    line = actuator.readline().decode().strip()    
    if line == '':
        return -1
    elif line is None:
        return -1 
    elif line:  # Check if data was received
        return line
 # Indicate no data received within timeout
